...
daq(type: json) is too big to transfer with NATS 
"""
import re
import os
...
async def publish_main():
    ...
    for i in generatorObj:
        transferSlice = dataset.loc[dataset['second'] == df_start_sec + i].to_json(orient='records', double_precision=15)
        await nc.publish("milling", transferSlice.encode())
    await nc.close()
